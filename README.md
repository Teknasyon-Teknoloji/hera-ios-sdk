...
After loading the ad you can show it by using the `showAd(ofType:for:on:)` method provided with the ad type and its corresponding action. Please note that this method may trigger various events, according to the ad showing state, like` didShowAd`, `didFailToShow`, `didDismissAd`  and so on.
